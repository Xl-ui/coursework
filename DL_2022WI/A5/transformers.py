...
def generate_token_dict(vocab):
    ...
    token_dict = {s:i  for i,s in enumerate(vocab) }
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return token_dict
...
def prepocess_input_sequence(
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    input_str_token=input_str.split()
    for s in input_str_token:
        if s in spc_tokens:
            out.append(token_dict[s])
        else:
            for digit_str in s:
                out.append(token_dict[digit_str])
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_two_loop_single(
    ...
    K,M=query.shape
    scores = torch.empty(K,K,dtype=query.dtype,device=query.device)
    for i in range(K):
        for j in range(K):
            scores[i,j]=query[i].dot(key[j])/M**0.5
    weights=F.softmax(scores,dim=1)
    out=torch.mm(weights,value)
    """notes:
    0.注意scores和weights的形状是K*K,代表序列中各个token之间的相似度
    1.torch.sqrt()接收一个张量,逐元素施加sqrt(),因此对于int类型的M,
    不能使用torch.sqrt(M).可以创建相应的零维张量torch.tensor(M)再使用torch.sqrt
    但更方便的是直接利用Python对幂次的运算符重载,即M**0.5
    2.注意内层循环遍历序列中的K个token,而不是遍历token的特征维度M
    3.循环实现中,out被原地修改了(通过[]索引),这会破坏计算图,实践中不会采用循环实现
    """
...
def scaled_dot_product_two_loop_batch(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    scores=torch.empty(N,K,K,dtype=query.dtype,device=query.device)
    for i in range(K):
        for j in range(K):
            scores[:,i,j]=torch.sum(query[:,i]*key[:,j],dim=1)/M**0.5
    weights=F.softmax(scores,dim=2)
    out=torch.bmm(weights,value)
    """notes:
    1.out[:,i,j]的计算:
    out[:,i,j]=torch.tensordot(query[:,i],key[:,j],dims=([1],[1]))/M**0.5
    是错误的,因为tensordot返回结果的形状是(N,N),但是out[:,i,j]需要的形状是(N,),
    我们只需要对应batch的query_i与key_j的内积,tensordot会把不同batch之间的
    query_i与key_j的内积也计算出来,实际上,tensordot返回结果的主对角线元素才是我们需要的
    2.query[:,i]与query[:,i,:]是等价的,Pytorch中省略尾部的:不影响结果.
    """
...
def scaled_dot_product_no_loop_batch(
    ...
    notes:
    1.需要传入mask的原因:
      encoder和decoder中都需要用到自注意力机制,
      encoder中不需要添加掩码,decoder中需要添加掩码
    2.在自注意力块中的位置:
      由后续的SelfAttention类管理Wq,Wk,Wv,并将输入投影到相应的query,key,value
      本方法只负责接收经过W投影后的query,key,value,使用缩放点积的方法+softmax,
      计算出权重分布,并返回根据该分布计算出的value的加权平均
    3.注意SelfAttention类允许dim_v与dim_q不同,只要求dim_q与dim_k相同(因为它们要做内积)
    该方法的文档注释中query,key,value的词向量维度是相同的,但实际实现
    并没有要求dim_v与dim_q相同,因此与SelfAttention是兼容的
    """
 ...
 #helper function: SelfAttention中Q,K,V的初始化
...
def init_weight(d_in, d_out):
    ...
    Args:
        d_in : nn.Linear的输入维度
        d_out : nn.Linear的输出维度
    Returns:
        (d_out,d_in)形状的权重; nn.Parameter类型
    """
    c = math.sqrt(6 / (d_in + d_out))
    return nn.Parameter(torch.rand(d_out, d_in) * 2 * c - c)

    #notes:d_in,d_out应该返回(d_out,d_in)形状的张量
    #因为Pytorch中nn.Linear的weight是实际W的转置,即形状为(d_out,d_in)
    #这么存储的原因是,在行优先布局下,此时同一个输出神经元所对应的权重向量在内存中是连续的
    #CPU/GPU读取时缓存命中率更高,计算更高效
...
class SelfAttention(nn.Module):
    def __init__(self, dim_in: int, dim_q: int, dim_v: int):
        ...
        self.q.weight=init_weight(dim_in,dim_q)
        self.k.weight=init_weight(dim_in,dim_q)
        self.v.weight=init_weight(dim_in,dim_v)
...
""" notes:
    SelfAttention块本身只要求投影后的Q,K维度相同,V维度可以与它们不同
    且不要求Q,K,V维度与输入维度相同
    而MultiHeadAttention指定投影后的Q,K,V维度相同
    
    SelfAttention和MultiHeadAttention的forward()都接收未经过投影的
    初始query,key,value(以下query,key,value都代表未经过投影的输入),
    然后在forward()内部对它们进行投影,并进行内积计算.
    这是因为,虽然decoder中query,key,value是相同的,但是
    执行cross-attention时,query是decoder中masked attention的结果
    key,value是encoder中经过若干个encoder_block后的输出
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        self.out=nn.Linear(num_heads*dim_out,dim_in)  #线性层
        self.out.weight=init_weight(num_heads*dim_out,dim_in)  #修改权重
        #最终输出维度是num_heads*dim_out , 一般等于dim_in
    ...
    def forward(
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        outputs=[single_head(query,key,value,mask) for single_head in self.heads]
...
class LayerNormalization(nn.Module):
    ...
    def forward(self, x: Tensor):
        ...
        last_dim=x.shape[-1]
        mean=torch.sum(x,dim=last_dim_index,keepdim=True)/last_dim
        centered=x-mean
        var=torch.sum(centered**2,dim=last_dim_index,keepdim=True)/last_dim
        std=(var+self.epsilon)**0.5
        normalized=centered/std
        y=self.gamma*normalized+self.beta
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
class FeedForwardBlock(nn.Module):
    def __init__(self, inp_dim: int, hidden_dim_feedforward: int):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        linear1=nn.Linear(inp_dim,hidden_dim_feedforward)
        linear1.weight=init_weight(inp_dim,hidden_dim_feedforward)

        linear2=nn.Linear(hidden_dim_feedforward,inp_dim)
        linear2.weight=init_weight(hidden_dim_feedforward,inp_dim)
        self.layer=nn.Sequential(linear1,nn.ReLU(),linear2)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class EncoderBlock(nn.Module):
    def __init__(
       ...
       inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \ 
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        """ notes:Hint的含义是指要使num_heads*dim_out==emb_dim """
        #MultiHeadAttention(num_heads,emb_dim,emb_dim//num_heads)
        #残差连接->layerNorm(emb_dim)
        #FeedForward(emb_dim,feedforward_dim)
        #残差连接->layerNorm(emb_dim)
        ...
        """
        notes:
        1.创建一个dropout实例即可,每次调用dropout实例的前向传播时,
        都会独立地给输入张量的每一个位置进行伯努利采样,以p的概率置零;
        不同次调用dropout()生成的掩码是完全独立的
        2.多头注意力实例模块需要命名为MultiHeadBlock
        否则后续.ipynb中# Visualize the attention weights的可视化代码会出错
        因为那里需要名为MultiHeadBlock的实例属性存在
        """
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
    ...
    def forward(self, x):
        ...
        #notes:原论文:sublayer->dropout->residual->layerNorm
        # out_immediate=self.ln1(self.dropout(self.multihead(x,x,x))+x)
        # y=self.ln2(self.dropout(self.ff(out_immediate))+out_immediate)
        # print("代码已经修改")
        out_immediate=self.dropout(self.ln1(self.MultiHeadBlock(x,x,x)+x))
        y=self.dropout(self.ln2(self.ff(out_immediate)+out_immediate))
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
def get_subsequent_mask(seq):
    ...
    # Replace "pass" statement with your code
    N,K=seq.shape
    mask=torch.triu(torch.ones(N,K,K,dtype=torch.bool,device=seq.device),diagonal=1)
    """notes:
    1.torch.triu(x,diagonal=0)  (triangular upper)
    保留主对角线及其上方的元素,剩余元素置为0
    torch.tril(x,diagonal=0) (triangular lower)
    保留主对角线及其下方的元素,
    2. dtype=torch.bool 生成布尔张量
    3. 指明device=seq.device,否则使用GPU时,seq在GPU上,mask在cpu上,会报错
    """
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
...
class DecoderBlock(nn.Module):
    ...
    def forward(
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        dec_out=self.attention_self(dec_inp,dec_inp,dec_inp,mask) #self attention,masked
        dec_out=self.norm1(dec_out+dec_inp)   #residual & layernorm
        dec_out=self.dropout(dec_out)  #dropout

        cross_out=self.attention_cross(dec_out,enc_inp,enc_inp) #cross attention,without mask
        cross_out=self.norm2(dec_out+cross_out) #residual & layernorm
        cross_out=self.dropout(cross_out) #dropout

        y = self.feed_forward(cross_out)  #feed forward
        y = self.norm3(y+cross_out) #residual & layernorm
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    #只看特征维度,指数序列是一样的;因此可以写成pos索引与指数序列的外积
    exponent = 2* torch.floor(torch.arange(M,dtype=torch.float32)/2)/M   # floor(i/2)得到组别
    pos = torch.arange(K,dtype=torch.float32).unsqueeze(1)
    angles = pos / torch.pow(10000.0,exponent)  #shape = (K,M)
    is_even = torch.arange(M)%2==0
    y = torch.where(is_even,torch.sin(angles),torch.cos(angles))
    y = y.unsqueeze(0)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return y
### note:原作业代码实现(注释与测试)有误: 2i/M无需向下取整
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        #note:decoder输入:要除去[BOS,...EOS]末尾的EOS
        #标签中,则要除去BOS,保留EOS
        # a_emb是三维张量; `:-1` 对应的是第二维,第三维度省略
        dec_out = None
        ##########################################################################
        ...
        mask = get_subsequent_mask(ans_b[:,:-1])  #注意用去除EOS后的张量形状来生成掩码
        dec_logits = self.decoder(a_emb_inp,enc_out,mask)
        # dec_out 中的值为 logits,且要转化为(N*O,M)的形状,以便后续调用cross_entropy计算损失
        dec_out = dec_logits.reshape(-1,dec_logits.shape[-1])
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class AddSubDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        args:
            ...
            convert_str_to_tokens: Dictionary to convert input string to tokens  #即token_dict
...
def LabelSmoothingLoss(pred, ground):
    ...
    """
    # pred: logits (N*O,V)
    # ground: (N,O) 每个元素是整数ID(0,..,V-1) 
    ground = ground.contiguous().view(-1)
    eps = 0.1  #把eps概率均分给类别分布中其它非正确标签的位置
    n_class = pred.size(1)
    one_hot = torch.nn.functional.one_hot(ground).to(pred.dtype)   # shape = (N*O,n_class
    #最好指明num_classes=n_class; 目前由于EOS是最大索引且总是出现在ground中,所以one-hot创建不会出问题
    one_hot = one_hot * (1 - eps) + (1 - one_hot) * eps / (n_class - 1)  # 平滑标签
    log_prb = F.log_softmax(pred, dim=1) 
    loss = -(one_hot * log_prb).sum(dim=1)  #计算每个token贡献的交叉熵损失
    loss = loss.sum()  #对所有token的交叉熵损失求和
    return loss
...
def CrossEntropyLoss(pred, ground):
    ...
    return loss
    # 注: 原注释错误:实际调用时(a5_helper.py:train()),ground的形状为(N*O,); 
    # LabelSmoothingLoss()中也是类似的,一开始的contiguous.view(-1)相当于是无效操作
