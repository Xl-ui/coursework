...
class Solver(object):
    ...
    def _step(self):
        ...
        if not hasattr(self,'counter'):
            ...
            self.counter+=1
            if self.counter>=iterations_per_epoch:
                for p, g in grads.items():
                    print(f"{p} grad norm: {g.norm().item():.6f}")
                for p, w in self.model.params.items():
                    print(f"{p} param norm: {w.norm().item():.6f}")
                self.counter=0
        """
    ...
    def train(self, time_limit=None, return_best_params=True):
        ...
        """
        #打印学习率配置
        #print('learning_rate=',self.optim_config['learning_rate'])
